...
@router.get("/")
async def list_locomotives(db: DbPool, offset: int = 0, limit: int = 50):
...
async def create_locomotive(data: LocomotiveCreate, db: DbPool):
...
async def get_locomotive(locomotive_id: str, db: DbPool):
