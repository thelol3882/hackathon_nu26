...
async def create_locomotive(
    session: AsyncSession, data: LocomotiveCreate
) -> LocomotiveRead:
    entity = Locomotive(
        id=generate_id(),
...
async def get_locomotive(
    session: AsyncSession, locomotive_id: str
) -> LocomotiveRead:
...
async def list_locomotives(
    ...
    result = await session.execute(
        select(Locomotive)
        .order_by(Locomotive.created_at.desc())
        .offset(offset)
        .limit(limit)
    )
    return [
        LocomotiveRead.model_validate(row, from_attributes=True)
