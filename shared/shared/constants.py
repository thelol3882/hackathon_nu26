...
_TE33A: dict[str, SensorSpec] = {
    SensorType.DIESEL_RPM: SensorSpec(
        weight=15, p_nom=700, delta_safe=200, p_crit=1050,
        threshold_type=ThresholdType.UPPER_BOUND, k=2.0,
    ),
    # Oil pressure is cross-validated against RPM (see alert_evaluator)
    SensorType.OIL_PRESSURE: SensorSpec(
        weight=35, p_nom=3.5, delta_safe=1.0, p_crit=1.5,
        threshold_type=ThresholdType.LOWER_BOUND, k=2.0,
    ),
    SensorType.COOLANT_TEMP: SensorSpec(
        weight=20, p_nom=82, delta_safe=13, p_crit=95,
        threshold_type=ThresholdType.UPPER_BOUND, k=2.0,
    ),
    SensorType.FUEL_LEVEL: SensorSpec(
        weight=10, p_nom=50, delta_safe=40, p_crit=5,
        threshold_type=ThresholdType.LOWER_BOUND, k=1.5,
    ),
    SensorType.FUEL_RATE: SensorSpec(
        weight=10, p_nom=80, delta_safe=60, p_crit=200,
        threshold_type=ThresholdType.UPPER_BOUND, k=1.5,
    ),
    SensorType.TRACTION_MOTOR_TEMP: SensorSpec(
        weight=15, p_nom=85, delta_safe=45, p_crit=130,
        threshold_type=ThresholdType.UPPER_BOUND, k=2.0,
    ),
    SensorType.CRANKCASE_PRESSURE: SensorSpec(
        weight=40, p_nom=0, delta_safe=10, p_crit=50,
        threshold_type=ThresholdType.UPPER_BOUND, k=3.0,
    ),
    # Common params shared across both types
    SensorType.BRAKE_PIPE_PRESSURE: SensorSpec(
        weight=40, p_nom=5.1, delta_safe=0.2, p_crit=2.0,
        threshold_type=ThresholdType.LOWER_BOUND, k=3.0,
    ),
    SensorType.WHEEL_SLIP_RATIO: SensorSpec(
        weight=20, p_nom=0.0, delta_safe=0.05, p_crit=0.5,
        threshold_type=ThresholdType.UPPER_BOUND, k=2.0,
    ),
    SensorType.SPEED_ACTUAL: SensorSpec(
        weight=15, p_nom=60, delta_safe=60, p_crit=120,
        threshold_type=ThresholdType.UPPER_BOUND, k=2.0,
    ),
}
...
_KZ8A: dict[str, SensorSpec] = {
    SensorType.CATENARY_VOLTAGE: SensorSpec(
        weight=25, p_nom=25_000, delta_safe=4_000, p_crit=19_000,
        threshold_type=ThresholdType.BIDIRECTIONAL, k=2.0,
    ),
    SensorType.PANTOGRAPH_CURRENT: SensorSpec(
        weight=15, p_nom=200, delta_safe=150, p_crit=400,
        threshold_type=ThresholdType.UPPER_BOUND, k=2.0,
    ),
    # Alstom transformer — cellulose aging doubles every 6 °C (Montsinger)
    SensorType.TRANSFORMER_TEMP: SensorSpec(
        weight=20, p_nom=65, delta_safe=15, p_crit=90,
        threshold_type=ThresholdType.UPPER_BOUND, k=2.0,
        is_aging_param=True, montsinger_ref_temp=65.0,
    ),
    # IGBT — semiconductor aging; critical if sustained overtemp
    SensorType.IGBT_TEMP: SensorSpec(
        weight=30, p_nom=57, delta_safe=18, p_crit=85,
        threshold_type=ThresholdType.UPPER_BOUND, k=2.0,
        is_aging_param=True, montsinger_ref_temp=57.0,
    ),
    SensorType.RECUPERATION_CURRENT: SensorSpec(
        weight=5, p_nom=100, delta_safe=100, p_crit=0,
        threshold_type=ThresholdType.LOWER_BOUND, k=1.0,
    ),
    SensorType.DC_LINK_VOLTAGE: SensorSpec(
        weight=20, p_nom=2_800, delta_safe=300, p_crit=2_000,
        threshold_type=ThresholdType.BIDIRECTIONAL, k=2.0,
    ),
    # Common params
    SensorType.BRAKE_PIPE_PRESSURE: SensorSpec(
